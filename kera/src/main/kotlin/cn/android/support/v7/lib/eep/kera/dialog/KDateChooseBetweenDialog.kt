...
open class KDateChooseBetweenDialog(context: Activity, var dateChooseStart: KDateChoose = KDateChoose(), var dateChooseEnd: KDateChoose = KDateChoose(), isStatus: Boolean = true, isTransparent: Boolean = true) : KDialog(context, R.layout.kera_dialog_date_choose_between,isStatus,isTransparent,isInitUI = true) {
    ...
    open fun updateDays2() {
        //日，联动，更加月份而改变
        var list_dd = ArrayList<String>()
        val mDay = KTimeUtils.getInstance().getMonthDay(yyyy2.currentItemValue + "-" + MM2.currentItemValue)//天数
        for (i in 1..mDay) {
            list_dd.add(i.toString())
        }
    ...
    //日期返回回调
    open fun setCallBack(callbak: (dateChooseStart: KDateChoose, dateChooseEnd: KDateChoose) -> Unit): KDateChooseBetweenDialog {
        //完成
        findViewById<View>(R.id.crown_txt_ok).setOnClickListener {
